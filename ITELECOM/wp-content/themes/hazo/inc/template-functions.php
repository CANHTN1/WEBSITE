 ...
 * @return array
 */
function hazo_body_classes($classes)
{
...
add_filter('body_class', 'hazo_body_classes');

/**
 * Add a pingback url auto-discovery header for single posts, pages, or attachments.
 */
function hazo_pingback_header()
{
	if (is_singular() && pings_open()) {
		printf('<link rel="pingback" href="%s">', esc_url(get_bloginfo('pingback_url')));
	}
}
add_action('wp_head', 'hazo_pingback_header');
...
function hazo_pagination()
{
	...
	echo '<div class="hazo-pagination"><ul>' . "\n";

	/** Previous Post Link */
	if (get_previous_posts_link())
		printf('<li>%s</li>' . "\n", get_previous_posts_link(svg('arrow-left')));
	...
	if (get_next_posts_link())
		printf('<li>%s</li>' . "\n", get_next_posts_link(svg('arrow-right')));
